 ...
 */
class CustomerPageActions extends Column
{
    /** Url path */
    // 'admin_route_name/folder_name/file_name'
    const CMS_URL_PATH_EDIT = 'news1/customer/indexform';
    const CMS_URL_PATH_DELETE = 'news1/customer/delete';
        ...
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                if (isset($item['customer_id'])) {
                    ...
                    $title = $this->getEscaper()->escapeHtml($item['first_name']);
                    $item[$name]['delete'] = [
                        'href' => $this->urlBuilder->getUrl(self::CMS_URL_PATH_DELETE, ['customer_id' => $item['customer_id']]),
                        'label' => __('Delete'),
                        'confirm' => [
